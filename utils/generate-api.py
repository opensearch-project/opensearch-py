...
class Module:
    ...
    def dump(self):
        ...
        print("Current execution path:", current_path)
        with open(init_file_path, "r") as f:
            content1 = f.read()
        if "delete_point_in_time" in content1:
            delete_pit_exists = True

        if delete_pit_exists == False:
            code_string = '''
@query_params()
...
def read_modules():
    ...
    for key, value in groupby(list_of_dicts, key=itemgetter("x-operation-group")):
        ...
        if name == "create_pit":
            ...
            api["params"].update({"ignore_unavailable": {'type': 'boolean', 'description': 'Whether specified concrete indices should be ignored when unavailable (missing or closed).'}})
            api["url"]["paths"].append({'path': '/_search/point_in_time', 'methods': ['POST']})

        # if name == "delete_pit":
        #     name=" delete_point_in_time"
        #     api["body"].pop("required")
        #     api["body"].update({"description":"a point-in-time id to delete"})
        #     # api.update({"params":{"all":{'type': 'boolean', 'description': 'set it to `True` to delete all alive point in time.'}}})

        #     # #api["url"]["paths"].append({'path': '/_search/point_in_time/_all', 'methods': ['DELETE']})
        #     api["url"]["paths"][0].update({'parts': {'all': {'type': 'boolean','description': 'set it to `True` to delete all alive point in time.'}}})
